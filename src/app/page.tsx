...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  const description = query
    ? `Explora en ${siteConfig.name} resultados para ${query} en sillas, estaciones y mobiliario profesional para salon y barberia.`
  ...
  const canonical = query || categoryFilter
    ? getSiteUrl(
        `/?${new URLSearchParams({
          ...(query ? { q: query } : {}),
...
export default async function HomePage({ searchParams }: PageProps) {
  ...
  const [products, systemCurrency, totalProducts, totalCategories] = await Promise.all([
  ...
  const categoriesCarousel = Array.from(
    products.reduce(
      (acc, product) => {
        ...
        const current = acc.get(product.categoryId);
  ...
  const homeSchema = {
    ...
    "@graph": [
      ...
      {
        "@type": "CollectionPage",
        "@id": `${getSiteUrl("/")}#catalog`,
        url: getSiteUrl("/"),
        name: `Catalogo de ${siteConfig.name}`,
        description:
                ...
                <div className="grid items-center gap-2.5 md:grid-cols-[minmax(0,1.02fr)_minmax(300px,0.98fr)] md:gap-6">
                  <div className="space-y-2.5 text-left md:space-y-4">
                    ...
                    <p className="w-full max-w-[19.5rem] text-[13px] leading-[1.45] text-white/80 md:max-w-lg md:text-base md:leading-6">
                      Disenado para negocios exigentes que quieren proyectar presencia, orden y confianza desde el
                      primer vistazo.
                    </p>
          ...
          {!query ? (
            <div className="rounded-2xl border border-[var(--line)] bg-white px-4 py-4 shadow-sm">
              ...
              <p className="mt-2 max-w-4xl text-sm leading-6 text-slate-600">
                En {siteConfig.name} reunimos mobiliario profesional pensado para negocios que necesitan proyectar
                calidad desde el primer vistazo. Encuentra sillas para salon de belleza, estaciones de trabajo,
                mobiliario para barberia y piezas seleccionadas para espacios que buscan una experiencia premium.
              </p>
